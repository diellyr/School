...
export function DevelopmentPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {!studentId && <EmptyState icon={Compass} title="Selecione um aluno" description="Use os filtros acima para escolher escola, turma e aluno da Educação Infantil." />}
          ...
          {data.analysis.unclassified.length > 0 && (
            <details className="rounded-lg border border-dashed border-slate-300 p-3 text-sm text-slate-500 dark:border-slate-700">
              <summary className="cursor-pointer font-medium">
                {data.analysis.unclassified.length} atividade(s) avaliada(s) sem habilidade correspondente no catálogo pedagógico
              </summary>
              <ul className="mt-2 list-disc space-y-0.5 pl-5">
                {data.analysis.unclassified.map((u) => (
                  <li key={u.activityId}>{u.activityTitle} — {u.period} ({u.rboLevel})</li>
                ))}
              </ul>
            </details>
          )}
                ...
                onAddToPlan={async (activity: FamilyActivity, reason: string) => {
                  ...
                  if (plan) {
                    ...
                    plan = await repositories.weeklyPlans.create(
                      { studentId, weekStart: new Date().toISOString().slice(0, 10), items: [newItem], pedagogicalRulesVersion: (await repositories.pedagogical.getMetadata()).version },
                      actor,
                    );
                  }
                  await repositories.activityHistory.create(
                    {
                      studentId, activityId: activity.id, skillId: analysis.skill.id, experienceFieldId: analysis.experienceFieldId,
                      weeklyPlanId: plan.id, weeklyPlanItemId: newItem.id, recommendationReason: reason,
